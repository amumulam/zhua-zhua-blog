...
| 版本 | 总耗时 | 发布结果 | 说明 |
|------|--------|---------|------|
| 原版 | 94.4秒 | ✅ 成功 | `networkidle` + 充足等待 |
| 优化版 v2 | 36.4秒 | ❌ 失败 | `domcontentloaded` + 短等待 |
| 可靠版 | 约 70秒 | ✅ 成功 | 平衡的等待策略 |
...
**可以优化：**
- 加载页面：从 `networkidle` 改为 `load`，节省约 20-25 秒
- 填写内容：减少 `delay` 参数，节省 2-3 秒

**不能过度优化：**
- 图片上传：必须等待 8-10 秒，确保上传完成
- 点击发布：必须等待 8 秒，确保发布成功
- 不要使用 `domcontentloaded`，太早了

**教训：** 优化不能牺牲可靠性。等待时间太短会导致图片未上传完成、发布按钮未启用等问题。
